...
mount_and_boot() {
    ...
    elif grep -q -w "overlay" /proc/filesystems; then
	...
	"overlayfs")
	    ...
	    ;;
    "overlay")
        MOUNTPOINTS="/rootfs.ro /volatile"

        #Create the necessary mount points for our overlayfs:
        #-A mount point for the read-only core filesystem;
        #-A mount point for the writable "overlay; and
        #-A mount point for the workdir.
	    mkdir -p $MOUNTPOINTS

        #Try to move the root mountpoint to the "lower" directory for our
        #overlay.
	    if ! mount -n --move $ROOT_MOUNT /rootfs.ro; then
            rm -rf $MOUNTPOINTS
            fatal "Could not move rootfs mount point"
            ...
            #Create a volatile mountpoint for the upper filesytem
            #and work directory. Note that both the work and upper
            #filesystem /must/ be on the same tmpfs for this to work.
            mount -t tmpfs -o rw,noatime,mode=755 tmpfs /volatile

            #... and create upper and work directories under that volatile image.
            mkdir -p /volatile/rootfs.rw /volatile/rootfs.work

            #Mount the drive itself...
            mount -t overlay -o "lowerdir=/rootfs.ro,upperdir=/volatile/rootfs.rw,workdir=/volatile/rootfs.work" overlay $ROOT_MOUNT

            #... and move our work into the roofs mount.
            mkdir -p $ROOT_MOUNT/rootfs.ro $ROOT_MOUNT/rootfs.rw
            mount --move /rootfs.ro $ROOT_MOUNT/rootfs.ro
            mount --move /volatile/rootfs.rw $ROOT_MOUNT/rootfs.rw
